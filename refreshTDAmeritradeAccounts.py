...
def lambda_handler(event, context):
    ...
    td_ameritrade_accounts = dynamoClient.scan(
        ExpressionAttributeNames=expression_attribute_names,
        ProjectionExpression=projection_expression,
        TableName='tdAmeritradeAccount'
    ).get("Items",[])
    try:
        for account in td_ameritrade_accounts:
            userId = getAttribute(account, 'userId', 'S')
            print("Refreshing for user: {}".format(userId))
            refresh_url = "https://ji1g9w5p36.execute-api.us-west-1.amazonaws.com/test/tdameritrade/refreshdata?userId={}".format(userId)
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Finished Refreshing TD Accounts')
    }