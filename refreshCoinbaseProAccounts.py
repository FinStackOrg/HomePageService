...
def lambda_handler(event, context):
    ...
    projection_expression = "#UID, #AK"
    coinbase_pro_accounts = dynamoClient.scan(
        ExpressionAttributeNames=expression_attribute_names,
        ProjectionExpression=projection_expression,
        TableName='coinbaseProAccount'
    ).get("Items",[])

    try:
        for account in coinbase_pro_accounts:
            api_key = getAttribute(account, 'api_key', 'S')
            userId = getAttribute(account, 'userId', 'S')
            print("Refreshing for user: {}".format(userId))
            refresh_url = "https://ji1g9w5p36.execute-api.us-west-1.amazonaws.com/test/coinbase/refreshdata/{}".format(api_key)
